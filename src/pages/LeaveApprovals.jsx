...
export default function LeaveApprovals() {
  ...
  const leaveTypes = useMemo(
    () => Array.from(new Set(ordered.map((r) => r.leaveType).filter(Boolean))),
    [ordered],
  );
  ...
  const { page, setPage, pageItems, pageSize } = usePagination(
    filteredOrdered,
    6,
  );
  ...
  const handleExport = () => {
    ...
    const rows = filteredOrdered.map((row) => [
      row.employeeName || "",
      row.leaveType || "",
      row.startDate || "",
      row.endDate || "",
    ...
    <div className="page-stack leave-approvals-page page-reveal">
      ...
      <AnimatePresence>
        {msg && (
          <motion.div
            ...
            transition={{ duration: 0.3, ease: easeOut }}
            onAnimationComplete={() => {
              if (msg) window.setTimeout(() => setMsg(""), 3500);
            }}
          >
      ...
      >
        {SUMMARY_CARDS.map((card) => (
          <motion.div
            key={card.label}
            className={`la-overview-card tone-${card.tone}`}
            variants={fadeUp}
            whileHover={{ y: -6 }}
          >
            ...
            <tbody>
              {loading &&
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={`lsk-${i}`} className="skeleton-row">
                    <td colSpan={7}>
              ...
              {!loading &&
                pageItems.map((row, i) => (
                  ...
                  >
                    ...
                    <td>
                      <span className="lt-type">{row.leaveType}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <span
                        className={`status-pill ${String(row.status || "").toLowerCase()}`}
                      >
          ...
          >
            ...
            >
              ...
              <p>
                ...
                <strong>{approveTarget.employeeName}</strong>'s{" "}
                <strong>{approveTarget.leaveType}</strong> Leave From{" "}
              ...
              <div className="modal-actions">
                ...
                >
                  <Check size={16} /> {acting ? "Approving..." : "Yes, approve"}
                </button>
              </div>
            </motion.div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <p>
                ...
                <strong>{rejectTarget.employeeName}</strong>'s{" "}
                <strong>{rejectTarget.leaveType}</strong> Leave Request.
              </p>
              ...
              <div className="modal-actions">
                ...
                <button
                  className="btn btn-gradient-red"
                  disabled={acting}
            ...
            >
              ...
              <div className="drawer-body">
                <div className="drawer-emp">
                  <span className="emp-avatar lg">
                    {initials(drawerItem.employeeName)}
                  </span>
                  <div>
                ...
                <div className="drawer-grid">
                  ...
                  <div className="dg-item">
                    ...
                    <strong>{drawerItem.leaveType}</strong>
                  </div>
                  <div className="dg-item">
                  ...
                  <div className="dg-item">
                    ...
                    <strong>{drawerItem.totalDays}</strong>
                  </div>
                  <div className="dg-item">
                    <Building2 size={15} />
                    <span>Department</span>
                    <strong>{drawerItem.department || "—"}</strong>
                  </div>
                  <div className="dg-item">
                    <ClipboardList size={15} />
                    <span>Status</span>
                    <strong>
                      <StatusBadge>{drawerItem.status}</StatusBadge>
                    </strong>
                  </div>
                </div>
                  ...
                  <div className="timeline">
                    <div className="tl-item">
                      ...
                      <div>
                        ...
                        <small>{drawerItem.startDate}</small>
                      </div>
                    </div>
                    <div className="tl-item">
                      <span className="tl-dot pending" />
                      <div>
                        <strong>Awaiting Manager Decision</strong>
                        <small>{drawerItem.status}</small>
                      </div>
                    </div>
                  </div>
                </div>
