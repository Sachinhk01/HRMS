...
export default function Dashboard() {
    ...
    async function loadBirthdays() {
      ...
      try {
        ...
        birthdayNotifs.forEach((bn) => {
          if (
            !combined.some((item) => item.id === bn.id || item.name === bn.name)
          ) {
            combined.push(bn);
          }
        });
  ...
  useEffect(() => {
    ...
    async function loadHolidays() {
      ...
      try {
        ...
        const upcoming = Array.isArray(upcomingResult)
          ? upcomingResult.filter(Boolean)
          : [];
  ...
  useEffect(() => {
    ...
    async function loadLeave() {
      ...
      try {
        ...
        const taken = (balances || []).reduce(
          (sum, b) => sum + (b.usedLeaves || 0),
          0,
        );
        const left = (balances || []).reduce(
          (sum, b) => sum + (b.remainingLeaves || 0),
          0,
        );
  ...
  if (role === "EMPLOYEE") {
    cards = [
      ...
      [
        CalendarDays,
        "Leaves Left",
        leaveLoading ? "..." : leaveSummary.left,
        `${leaveLoading ? "..." : leaveSummary.taken} Days Taken`,
        "pink",
        "/leave",
      ],
      [
    ...
    cards = [
      [
        ClipboardCheck,
        "Pending Leave",
        leaveLoading ? "..." : pendingApprovalsCount,
        "Awaiting Decision",
        "pink",
        "/leave-approvals",
      ],
      ...
      [
        Clock3,
        "Attendance",
        attendanceLoading ? "..." : attendanceCount,
        "Team Records",
        "green",
        "/attendance",
      ],
  ...
  function updateMagazine(event) {
    ...
    try {
      ...
      setMagazine(next);
      setMagazineFile({
        url: next.documentUrl || "",
        name: next.documentName || "",
        size: next.documentSize || 0,
      });
  ...
  function updateEmployeeOfMonth(event) {
    ...
    const selected = employees.find(
      (item) => String(item.id) === form.get("employeeId"),
    );
    try {
  ...
  return (
    <div className="page-stack dashboard-page page-reveal">
        ...
        <div className="welcome-banner-text">
          ...
          <p>
            {role === "EMPLOYEE"
              ...
              : role === "HR_ADMIN"
                ? "Manage People, Engagement And HR Operations."
                : "Review Team Attendance, Leave And Performance."}
          </p>
        </div>
      ...
      <div className="summary-grid">
        {cards.map(([Icon, label, value, meta, tone, path]) => (
          <SummaryCard
            key={label}
            icon={Icon}
            label={label}
      ...
      >
        ...
        <div className="dashboard-holiday-content">
            ...
            <div className="dashboard-holiday-list">
              {upcomingHolidays.slice(0, 3).map((holiday) => (
                <div
                  ...
                  className="dashboard-holiday-chip"
                >
                  <span>
                    {new Date(
          ...
          {holidaysError && (
            <p className="dashboard-holiday-error">{holidaysError}</p>
          )}
        </div>
        ...
        <section className="panel highlights-editor">
          <div className="highlights-editor-head">
            <div>
              ...
              <p>
                Whatever You Publish Here Replaces Last Month's — Every Employee
                Sees It The Moment You Save.
              </p>
            </div>
            <div className="highlights-editor-badge">
              <Sparkles size={14} /> Live This Month
            </div>
          </div>
          ...
          {message && (
            <div
              className={
                message.includes("updated") ? "success-alert" : "form-alert"
              }
            >
              {message}
            </div>
          ...
          <div className="highlights-editor-grid">
            <form
              className="editor-card editor-card--blue"
              onSubmit={updateMagazine}
            >
              <div className="editor-card-head">
                <div className="editor-card-icon editor-card-icon--blue">
                  <BookOpen size={18} />
                </div>
                <div>
                  <h3>Monthly Magazine</h3>
                  <p>
                    PDF Opens or Downloads For Every Employee On Their Dashboard
                  </p>
                </div>
              </div>
              ...
              <div className="editor-fields">
                <label className="ef-field ef-full">
                  ...
                  <input
                    ...
                    placeholder="e.g. MyHourly Times — August Edition"
                    required
                  />
                </label>
                ...
                <label className="ef-field">
                  ...
                  <input
                    name="coverUrl"
                    type="url"
                    defaultValue={magazine?.coverUrl || ""}
                    placeholder="https://…"
                  />
                </label>
                <label className="ef-field ef-full">
              ...
              <button className="btn btn-primary btn-block">
                <Save size={17} /> Save &amp; Publish Magazine
              </button>
            </form>

            <form
              className="editor-card editor-card--warm"
              onSubmit={updateEmployeeOfMonth}
            >
              <div className="editor-card-head">
                <div className="editor-card-icon editor-card-icon--warm">
                  <Award size={18} />
                </div>
                <div>
              ...
              <div className="editor-fields">
                <label className="ef-field ef-full">
                  ...
                  <select
                    name="employeeId"
                    defaultValue={employeeOfMonth?.employeeId || ""}
                    required
                  >
                ...
                <label className="ef-field">
                  ...
                  <input
                    name="month"
                    type="month"
                    defaultValue={employeeOfMonth?.month || ""}
                    required
                  />
                </label>
                <label className="ef-field">
                  ...
                  <input
                    name="photoUrl"
                    type="url"
                    defaultValue={employeeOfMonth?.photoUrl || ""}
                ...
                <label className="ef-field ef-full">
                  ...
                  <textarea
                    name="message"
                    rows="4"
                    defaultValue={employeeOfMonth?.message || ""}
              ...
              <button className="btn btn-primary btn-block">
                <Award size={17} /> Publish Recognition
              </button>
            </form>
          </div>
        </section>
